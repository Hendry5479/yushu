...
class ChangePasswordForm(Form):
    old_password = PasswordField(validators=[DataRequired('密码不能为空'), Length(6, 32)])

    new_password1 = PasswordField(validators=[DataRequired('密码不能为空，请输入密码'), Length(6, 32, message='密码长度需要在6到32位之间'),
                                              EqualTo('new_password2', message='两次输入的密码不一致')])

    new_password2 = PasswordField(
        validators=[DataRequired('确认密码不能为空，请输入密码'), Length(6, 32, message='确认密码长度需要在6到32位之间')])
